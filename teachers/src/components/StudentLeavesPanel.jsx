...
export function NfcLeaveScan({ onScan, t }) {
  ...
  async function runScan(raw) {
    ...
    if (!value) {
      setTone('bad')
      setMessage(t('nfcLeaveNeedCode'))
      return
    }
    setBusy(true)
    setMessage('')
    try {
      const result = await onScan?.(value, leaveType)
      if (result?.ok) {
        setTone('ok')
        setMessage(
        ...
        setTone('bad')
        setMessage(result?.error || t('nfcLeaveFail'))
      }
    } catch (err) {
      setTone('bad')
      setMessage(err.message || t('nfcLeaveFail'))
    } finally {
      setBusy(false)
    }
  }
  ...
  async function startNfcListen() {
    ...
    try {
      ...
      setListening(true)
      setMessage(t('nfcLeaveListening'))
      setTone('ok')
      ...
      setListening(false)
      setTone('bad')
      setMessage(err.message || t('nfcLeaveNoWebNfc'))
    }
  }

  function stopNfcListen() {
    try {
      abortRef.current?.abort()
    } catch {
      /* ignore */
  ...
  return (
    <div className="nfc-leave-scan">
      <p className="muted">{t('nfcLeaveHint')}</p>
      <label className="field">
        <span>{t('nfcLeaveType')}</span>
        <select value={leaveType} onChange={(e) => setLeaveType(e.target.value)} disabled={busy}>
      ...
      <label className="field">
        <span>{t('nfcLeaveCode')}</span>
        <input
          value={code}
          onChange={(e) => setCode(e.target.value)}
          placeholder={t('nfcLeaveCodePh')}
      ...
      <div className="row-actions">
        <button
          type="button"
          className={`primary${busy ? ' is-loading' : ''}`}
          disabled={busy}
          onClick={() => runScan(code)}
        >
          {busy ? <span className="btn-spinner" aria-hidden="true" /> : null}
          <span>{busy ? t('working') : t('nfcLeaveRecord')}</span>
        </button>
        {nfcSupported ? (
          <button type="button" className="ghost" disabled={busy} onClick={listening ? stopNfcListen : startNfcListen}>
            {listening ? t('nfcLeaveStop') : t('nfcLeaveStart')}
          </button>
        ) : null}
      </div>
      {!nfcSupported ? <p className="muted nfc-leave-fallback">{t('nfcLeaveManualOnly')}</p> : null}
      {message ? <p className={`nfc-leave-msg ${tone === 'bad' ? 'bad' : 'ok'}`}>{message}</p> : null}
      <p className="muted" style={{ fontSize: '0.78rem' }}>{t('nfcLeaveSecondScan')}</p>
    </div>
  )
}
